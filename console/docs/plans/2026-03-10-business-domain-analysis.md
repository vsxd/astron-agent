...
## 六、原 Toolkit HTTP 调用分析（合并后需替换为直接调用）

> **背景：** toolkit 模块合并入 hub 之前，hub 通过 HTTP API 调用 toolkit 的部分功能。合并后这些 HTTP 调用应替换为直接的 Service 注入调用，消除不必要的网络开销和部署依赖。

### 6.1 高优先级替换（原 toolkit 核心功能）

#### 6.1.1 ToolServiceCallHandler — 工具管理调用

**文件:** `hub/handler/ToolServiceCallHandler.java`
**配置:** `api.url.toolUrl` = `${TOOL_URL:http://127.0.0.1:18888}`

| 方法 | HTTP 调用 | 端点 | 业务目的 | 替换目标 |
|------|-----------|------|----------|----------|
| `toolRun()` | `OkHttpUtil.post` | `/api/v1/tools/http_run` | 工具运行 | 直接调用 ToolBoxService |
| `toolDebug()` | `OkHttpUtil.post` | `/api/v1/tools/tool_debug` | 工具调试 | 直接调用 ToolBoxService |
| `toolCreate()` | `OkHttpUtil.post` | `/api/v1/tools/versions` | 创建工具版本 | 直接调用 ToolBoxService |
| `toolUpdate()` | `OkHttpUtil.put` | `/api/v1/tools/versions` | 更新工具版本 | 直接调用 ToolBoxService |
| `toolDelete()` | `OkHttpUtil.delete` | `/api/v1/tools/versions` | 删除工具版本 | 直接调用 ToolBoxService |

#### 6.1.2 CoreSystemService — 工作流引擎 + SparkDB 调用

**文件:** `hub/service/extra/impl/CoreSystemService.java`
**配置:** `api.url.workflow` = `${WORKFLOW_URL:http://127.0.0.1:7880}`, `api.url.sparkDB` = `${SPARK_DB_URL:http://127.0.0.1:7990}`

| 方法 | 端点 | 业务目的 | 替换目标 |
|------|------|----------|----------|
| `publish()` | `/v1/publish` | 工作流发布 | WorkflowProtocolService |
| `auth()` | `/v1/auth` | 工作流鉴权 | WorkflowProtocolService |
| `uploadFile()` | `/workflow/v1/upload_file` | 上传文件 | WorkflowProtocolService |
| `batchUploadFile()` | `/workflow/v1/upload_files` | 批量上传文件 | WorkflowProtocolService |
| `addComparisons()` | `/workflow/v1/protocol/compare/save` | 添加对比 | WorkflowProtocolService |
| `deleteComparisons()` | `/workflow/v1/protocol/compare/delete` | 删除对比 | WorkflowProtocolService |
| `createDatabase()` | `/xingchen-db/v1/create_database` | 创建数据库 | DatabaseService |
| `execDDL()` | `/xingchen-db/v1/exec_ddl` | 执行 DDL | DatabaseService |
| `execDML()` | `/xingchen-db/v1/exec_dml` | 执行 DML | DatabaseService |
| `uploadData()` | `/xingchen-db/v1/upload_data` | 上传数据 | DatabaseService |
| `cloneDatabase()` | `/xingchen-db/v1/clone_database` | 克隆数据库 | DatabaseService |
| `dropDatabase()` | `/xingchen-db/v1/drop_database` | 删除数据库 | DatabaseService |
| `modifyDataBase()` | `/xingchen-db/v1/modify_db_description` | 修改数据库描述 | DatabaseService |

#### 6.1.3 WorkflowService — 工作流协议操作

**文件:** `hub/service/workflow/WorkflowService.java`
**配置:** `api.url.workflow` = `${WORKFLOW_URL:http://127.0.0.1:7880}`

| 方法 | 端点 | 业务目的 | 替换目标 |
|------|------|----------|----------|
| `buildWorkflow()` | `/workflow/v1/protocol/build/{flowId}` | 构建工作流（SSE） | WorkflowProtocolService |
| `nodeDebug()` | `/workflow/v1/node/debug/` | 单节点调试 | WorkflowProtocolService |
| `logicDelete()` | `/workflow/v1/protocol/delete` | 删除工作流协议 | WorkflowProtocolService |
| `callProtocolAdd()` | `/workflow/v1/protocol/add` | 添加工作流协议 | WorkflowProtocolService |
| `saveRemote()` | `/workflow/v1/protocol/update/{flowId}` | 更新工作流协议 | WorkflowProtocolService |
| `runCode()` | `/workflow/v1/run` | 运行代码节点 | WorkflowProtocolService |
| `sseChat()` | `/workflow/v1/debug/chat/completions` | 工作流对话调试（SSE） | WorkflowExecutionService |
| `sseChatResume()` | `/workflow/v1/debug/resume` | 工作流对话恢复（SSE） | WorkflowExecutionService |

#### 6.1.4 LocalModelHandler — 本地模型部署

**文件:** `hub/handler/LocalModelHandler.java`
**配置:** `api.url.localModel` = `${LOCAL_MODEL_URL:http://127.0.0.1:33778}`

| 方法 | 端点 | 业务目的 | 替换目标 |
|------|------|----------|----------|
| `getLocalModelList()` | `/api/v1/modserv/list` | 获取本地模型列表 | LocalModelService |
| `deployModel()` | `/api/v1/modserv/deploy` | 部署模型 | LocalModelService |
| `deployModelUpdate()` | `/api/v1/modserv/{serviceId}` | 更新模型部署 | LocalModelService |
| `checkDeployStatus()` | `/api/v1/modserv/{serviceId}` | 检查部署状态 | LocalModelService |
| `deleteModel()` | `/api/v1/modserv/{serviceId}` | 删除模型服务 | LocalModelService |

### 6.2 中优先级替换（数据集/知识库服务）

#### 6.2.1 FileInfoV2Service — 数据集文件操作

**文件:** `hub/service/data/impl/FileInfoV2Service.java`
**配置:** `api.url.datasetFileUrl`, `api.url.xinghuoDatasetFileUrl` 等

约 12 个 HTTP 调用，涉及数据集文件的上传、切片、向量化、删除等操作。应替换为直接调用 DatasetService。

#### 6.2.2 RepoService — 数据集操作

**文件:** `hub/service/repo/impl/KnowledgeRepoService.java`
约 3 个 HTTP 调用，涉及数据集创建、删除。应替换为直接调用 DatasetService。

#### 6.2.3 SparkKnowledgeCallHandler — 知识库调用

**文件:** `hub/handler/SparkKnowledgeCallHandler.java`
2 个 HTTP 调用，涉及知识库命中测试。应替换为直接调用 KnowledgeSearchService。

### 6.3 需保留的外部服务调用（非 toolkit）

以下 HTTP 调用是对真正的外部第三方平台，不应替换为直接调用，但建议统一封装为 `XxxClient` 防腐层：

| 调用方 | 外部服务 | 说明 |
|--------|----------|------|
| `McpServerHandler` | MCP Server | MCP 协议工具调用 |
| `MaasUtil` | MaaS 平台 | 工作流同步、模型配置 |
| `TenantServiceImpl` | 租户管理平台 | 租户信息查询 |
| `AppService` | 应用管理平台 | 应用信息查询 |
| `OpenPlatformService` | 开放平台 | 外部 API 集成 |
| `RpaHandler` | RPA 服务 | RPA 执行 |
| `ModelService` | 用户自定义模型端点 | 模型可用性验证 |
| `PromptChatService` | AI 模型 API | 对话调用（动态 URL） |
| `SparkApiTool` | 星火 API | WebSocket 对话 |
...
- `sparkDB` — SparkDB 地址
- `localModel` — 本地模型服务地址
- `knowledgeUrl` — 知识库服务地址
- `datasetUrl` / `datasetFileUrl` — 数据集服务地址
- `xinghuoDatasetFileUrl` / `deleteXinghuoDatasetFileUrl` / `deleteXinghuoDatasetUrl` — 星火数据集地址

**application.yml 中对应的环境变量配置** 和 **RestConfig 中的 RestTemplate Bean** 也可考虑移除。

---

## 七、重构优先级建议

> **注意：** 第一阶段新增"替换原 toolkit HTTP 调用"作为最高优先级项，因为这些调用在合并后已无必要且增加了部署复杂度。

### 第一阶段：解耦核心共享服务
1. **替换原 toolkit HTTP 调用为直接 Service 调用**（见第六章）— 最高优先级
2. 拆分 `ChatBotDataService` 为域内查询接口 + 跨域查询接口
